<#
    .SYNOPSIS
        You can feed this script a task sequece ts.xml (in this case it's the Serenity TS) and it parses all the
        software components the task sequence uses. Then the script pulls down from NEXUS the software components which
    ...
    .DESCRIPTION
        Example: 
            Local testing:
                ...
                ./component_retrieval.ps1 -mdt_build_tasklist "./ts.xml" -local_destination "C:\Users\zk787uq\Desktop\testing\Applications" -cacheFolder "VendorDeliveryCache" -verbose_logging

        Consideration:
            This script will be run on build server 123 (specifically on the D:\ drive) through a Jenkins job.
            
            cd E:\testing_1
            ./component_retrieval.ps1 -mdt_build_tasklist "./ts.xml" -local_destination "E:\Windows_10testing\Applications" -cacheFolder "VendorDeliveryCacheTesting" -verbose_logging

        Jenkins:
            cd ATMCICD-206
            ./component_retrieval.ps1 -mdt_build_tasklist "./ts.xml" -local_destination "E:\Windows_10testing\Applications" -cache Folder "VendorDeliveryCacheTesting" -unpackToDeploymentShare -verbose_logging
...
param (
    ...
    [Parameter(Mandatory = $false)][string]$local_destination = "C:\Users\zk787uq\Desktop\testing\Applications",
    [Parameter(Mandatory = $false)][string]$cacheFolder = "VendorDeliveryCache", 
    [Parameter(Mandatory = $false)][switch]$verbose_logging
    )

$Logfile = "C:\Users\ze787uq.CORP\Desktop"
...
function run_log_print([string]$log_message, [string]$log_level, [string]$return_code)
{
    ...
    {
        Write-Output "$log_date_time $log_level $computer_name $application_user $return_code $log_message"
        LogWrite "$log_date_time $log_level $computer_name $application_user $return_code $log message"
...
{
    Param (
        [Parameter(Mandatory = $true)] $finaluri, 
    ...
    try
    {
        ...
        {
            run_log_print -log_message "Unpacking=$artifactFileName Full_Directory=$local_destination\$target_directoy_name" -log_level INFO
            Expand-Archive -Path $cachedComponentZipLocation\$artifactFileName -DestinationPath $local_destination\$target_directoy_name -ErrorAction Stop | Out-Null
        }
    } 
    catch
    {
        run_log_print -log_message "Unable_to_Download=$artifactFileName." -log_level WARN
    }
}
...
function validate_installer_pattern($installer_name)
{
    ...
    {
        throw "'${installer name}' lacks a version number." 
        break
    }
    #
    ...
    elseif (($installer_name -split '(^Bank of America\s.*\s\d*\.\d*.*)')[1])
    {
        ...
        run_log_print -log_message "Application Name=$installer_application" -log_leve 1 DEBUG
    }
...
foreach ($installer in $bdd_install_applications)
{
    ...
    try
    {
        ...
        run_log_print -log_message "Artifact Match: Vendor=$( $artifact_info.vendor_name ) Application=$( $artifact_info.vendor_application ) Version=$( $artifact_info.vendor_application_version )" -log_level DEBUG
    }
    catch
    {
    ...
    #We call our 'check_artifact_already_present' function to see if it is already in our cache. 
    #If it is, we do not need to increase web domain traffic with the retrieval from NEXUS.
